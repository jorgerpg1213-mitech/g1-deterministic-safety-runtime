"""
Harness 4J-P2-A2 — FREEZE (Terminal Manual Path)
Workstream: B / F2 / R3 / E3 / C1 / P3

Hipótesis: SafetyEvent(rule_id=4F-P2-FREEZE, event_id=FIXED_ID)
           via ruta terminal R3 → recovery_g1
           → RecoveryEvent action_name=operator_intervention,
             recovery_type=REC-MANUAL, attempt_number=1,
             notes contiene parent_event_id=FIXED_ID

PASS:
  action_name=operator_intervention
  recovery_type=REC-MANUAL
  attempt_number=1
  notes contiene parent_event_id=4JP2-A2-FREEZE-001
...
class HarnessP2A2Freeze(Node):
    def __init__(self):
        super().__init__('harness_p2_a2_freeze')
        self._pub_time = None
        self._events   = []
        ...
        self.get_logger().info('[P2-A2-FREEZE] Harness iniciado')
        ...
        if self._pub_time is not None:
            if f'parent_event_id={FIXED_ID}' in (msg.notes or ''):
                ...
                self.get_logger().info(
                    f'[P2-A2-FREEZE] RecoveryEvent: action={msg.action_name} '
                    f'type={msg.recovery_type} attempt={msg.attempt_number} '
                    f'notes={msg.notes!r}')

def main():
    rclpy.init()
    node = HarnessP2A2Freeze()

    node.get_logger().info(f'[P2-A2-FREEZE] Warmup {WARMUP_S}s...')
    t0 = time.monotonic()
    rec_ready = False
    ...
    if subs != 1:
        print(f'FAIL — topología sucia: {subs} subscribers (esperado 1)')
        node.destroy_node(); rclpy.shutdown(); sys.exit(1)

    node.get_logger().info('[P2-A2-FREEZE] [Iniciado] — 1 subscriber en /safety_events')
    ...
    msg.notes                   = f'rule_id=4F-P2-FREEZE 4J-P2-A2 harness event_id={FIXED_ID}'
    msg.timestamp               = node.get_clock().now().to_msg()

    node._pub_time = time.monotonic()
    node.get_logger().warn(
        f'[P2-A2-FREEZE] Publicando SafetyEvent FREEZE event_id={FIXED_ID}')
    ...
    print('PASS — FREEZE terminal manual verificado')
