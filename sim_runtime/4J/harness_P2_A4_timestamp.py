"""
Harness 4J-P2-A4 — TIMESTAMP (Terminal Manual Path)
Workstream: B / F2 / R3 / E3 / C1 / P3

Hipotesis: SafetyEvent(rule_id=4F-P2-TIMESTAMP, event_id=FIXED_ID)
...
PASS:
  ...
  attempt_number=1
  notes contiene parent_event_id=4JP2-A4-TIMESTAMP-001
...
class HarnessP2A4Timestamp(Node):
    def __init__(self):
        super().__init__('harness_p2_a4_timestamp')
        self._pub_time = None
        self._events   = []
        ...
        self.get_logger().info('[P2-A4-TIMESTAMP] Harness iniciado')
        ...
        if self._pub_time is not None:
            if f'parent_event_id={FIXED_ID}' in (msg.notes or ''):
                ...
                self.get_logger().info(
                    f'[P2-A4-TIMESTAMP] RecoveryEvent: action={msg.action_name} '
                    f'type={msg.recovery_type} attempt={msg.attempt_number} '
                    f'notes={msg.notes!r}')

def main():
    rclpy.init()
    node = HarnessP2A4Timestamp()

    node.get_logger().info(f'[P2-A4-TIMESTAMP] Warmup {WARMUP_S}s...')
    t0 = time.monotonic()
    rec_ready = False
    ...
    node.get_logger().info('[P2-A4-TIMESTAMP] [Iniciado] -- 1 subscriber en /safety_events')
    ...
    msg.notes                   = f'rule_id=4F-P2-TIMESTAMP 4J-P2-A4 harness event_id={FIXED_ID}'
    msg.timestamp               = node.get_clock().now().to_msg()

    node._pub_time = time.monotonic()
    node.get_logger().warn(
        f'[P2-A4-TIMESTAMP] Publicando SafetyEvent TIMESTAMP event_id={FIXED_ID}')
    ...
    print('PASS -- TIMESTAMP terminal manual verificado')
