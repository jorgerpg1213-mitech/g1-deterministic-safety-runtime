...
class FaultInjector(Node):
    ...
    def _make_joint_sano(self):
        ...
        msg.position = [random.uniform(-0.1, 0.1) for _ in range(12)]
        msg.velocity = [random.uniform(-0.05, 0.05) for _ in range(12)]
        msg.effort   = [0.0] * 12
        return msg
    ...
    def _make_contact_sano(self):
        ...
        msg.header.stamp = self._now_stamp()
        msg.in_contact = True
        msg.force = 150.0 + random.uniform(-5.0, 5.0)
    ...
    def _publish_sano(self):
        if self.fault in ('imu_freeze', 'imu_stale', 'timestamp_regression'):
            msg = self._make_imu_sano()
            self._frozen_imu = msg
            s = msg.header.stamp
            self._last_stamp_sec = s.sec + s.nanosec * 1e-9
            self.pub.publish(msg)
        elif self.fault == 'joint_nan':
            self.pub.publish(self._make_joint_sano())
        elif self.fault == 'contact_freeze_false':
            self.pub.publish(self._make_contact_sano())
    ...
    def _publish_fault(self):
        ...
        if fault == 'imu_freeze':
            ...
            if self._fault_count == 0:
                ...
                self.pub.publish(self._make_imu_sano())
                self._fault_count += 1
