...
class Extension(omni.ext.IExt):
    ...
    async def _run_async(self):
        ...
        try:
            ...
            cr.initialize()
            import numpy as _np
            _ji={n:i for i,n in enumerate(robot.dof_names)}
            robot.set_world_poses(positions=_np.array([[0.0,0.0,0.72]],dtype=_np.float32), orientations=_np.array([[1.0,0.0,0.0,0.0]],dtype=_np.float32))
            _pose=_np.zeros((1,robot.num_dof),dtype=_np.float32)
            for _jn,_v in [("left_hip_pitch_joint",-0.1),("right_hip_pitch_joint",-0.1),("left_knee_joint",0.3),("right_knee_joint",0.3),("left_ankle_pitch_joint",-0.2),("right_ankle_pitch_joint",-0.2)]:
                _pose[0,_ji[_jn]]=_v
            robot.set_joint_positions(_pose)
            globals()["_P4G_NP"]=_np; globals()["_P4G_JI"]=_ji
            print("=== P4G BASELINE set (P2+z0.72), FALL TRIGGER en it=450 ===",flush=True)
    ...
    async def _publish(self, world, robot, node, names, pub_js, pub_pose, pub_vel, pub_imu, imu, cl, cr, pub_cl, pub_cr, rclpy):
        ...
        try:
            ...
            _np=globals().get("_P4G_NP"); _ji=globals().get("_P4G_JI"); _it4g=0; _fell=False; _t0=time.time()
            while rclpy.ok() and time.time() < t_end:
                _it4g+=1
                if _it4g in (1,100,250,449):
                    _q=robot.get_joint_positions()[0]; _p,_qt=robot.get_world_poses()
                    _terr=sum(abs(float(_q[_ji[k]])-v) for k,v in [("left_hip_pitch_joint",-0.1),("right_hip_pitch_joint",-0.1),("left_knee_joint",0.3),("right_knee_joint",0.3),("left_ankle_pitch_joint",-0.2),("right_ankle_pitch_joint",-0.2)])
                    print(f"P4G PRE it={_it4g} t={time.time()-_t0:.2f}s z={float(_p[0][2]):.4f} W={float(_qt[0][0]):.4f} joint_err_sum={_terr:.4f}",flush=True)
                if _it4g==450 and not _fell:
                    robot.set_world_poses(positions=_np.array([[0.0,0.0,1.10]],dtype=_np.float32), orientations=_np.array([[0.9238795,0.3826834,0.0,0.0]],dtype=_np.float32))
                    ...
                    _fell=True
                    print(f"=== P4G FALL TRIGGER it=450 t={time.time()-_t0:.2f}s = teleport(z1.10)+tilt45X+vel_reset, factory drives active ===",flush=True)
                world.step(render=False)
